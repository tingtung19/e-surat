@extends('layouts.app')
@section('content')
<div class="page-heading"><div><h1>{{ isset($letter) ? 'Edit Draft Surat' : 'Buat Surat' }}</h1><p>Lengkapi informasi surat dan lampiran sebelum dikirim.</p><div class="breadcrumb"><a href="{{ route('dashboard') }}">Beranda</a> &nbsp;/&nbsp; <a href="{{ route('letters.index') }}">Surat</a> &nbsp;/&nbsp; {{ isset($letter) ? 'Edit Draft' : 'Buat Surat' }}</div></div></div>
<div class="card form-card">
    ...
    <div class="card-body">
        <form method="post" action="{{ isset($letter) ? route('letters.update', $letter) : route('letters.store') }}" enctype="multipart/form-data">@csrf @if(isset($letter)) @method('PUT') @endif
            <div class="form-grid">
                <div class="form-group">
                    <label for="type">Jenis surat <span style="color:var(--red)">*</span></label>
                    <select id="type" name="type" required>
                        <option value="official" @selected(old('type', $letter->type ?? 'official') === 'official')>Resmi ke Direktur</option>
                        <option value="internal" @selected(old('type', $letter->type ?? null) === 'internal')>Internal antar divisi</option>
                        @if(auth()->user()->isAdmin())<option value="external" @selected(old('type', $letter->type ?? null) === 'external')>Surat masuk eksternal</option>@endif
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select id="target_division_id" name="target_division_id" data-default-division="{{ $defaultDivision?->id }}">
                        ...
                        @foreach($divisions as $division)<option value="{{ $division->id }}" @selected(old('target_division_id', $letter->target_division_id ?? null) == $division->id)>{{ $division->division_name ?? $division->name }}</option>@endforeach
                    </select>
                ...
                <div class="form-group full">
                    ...
                    <input id="title" name="title" required value="{{ old('title', $letter->title ?? '') }}" placeholder="Contoh: Permohonan Persetujuan Kegiatan">
                </div>
                <div class="form-group full">
                    <label for="description">Deskripsi / isi surat <span style="color:var(--red)">*</span></label>
                    <textarea id="description" name="description" required placeholder="Tuliskan isi surat secara lengkap...">{{ old('description', $letter->description ?? '') }}</textarea>
                </div>
                @if(auth()->user()->isAdmin())
                <div class="form-group full">
                    <label for="external_sender">Pengirim eksternal</label>
                    <input id="external_sender" name="external_sender" value="{{ old('external_sender', $letter->external_sender ?? '') }}" placeholder="Isi untuk surat yang berasal dari institusi luar">
                </div>
                @endif
                <div class="form-group full">
